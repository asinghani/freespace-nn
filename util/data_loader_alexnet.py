...
def prepare_data(X, Y, batch_size, augment_data, config = Config()):
    ...
    X = [preprocess_image(cv2.resize(x, (1135, 681)), config = config) for x in X]

    Y = [mpimg.imread(y) for y in Y]
    Y = [preprocess_label(cv2.resize(y, (1135, 681)), config = config) for y in Y]
...
########## NO RESIZE #######################################    
    ...
    def gen():
        ...
        while True:
            ...
            images = np.empty([batch_size, 227, 227, 3]) # change to 1 for greyscale
            ...
            for index in np.random.permutation(len(X)):
                ...
                patches = extract_patches_2d(image, (227, 227), max_patches=6)
                patches_img = patches[:, :, :, 0:3] # change to 0 and next line 1 for greyscale
                patches_label = patches[:, :, :, 3]

                for x in range(len(patches)):
                    images[i] = patches_img[x].reshape((227, 227, 3)) # change to 1 for greyscale
...
def postprocess(image, label, config = Config()):
    ...
    newLabel = np.zeros([681, 1135, 1], dtype=np.float32)
